	...
	for (PoiItem *item in ar_coordinates) {
		...
		if ([self viewportContainsCoordinate:item]) {
            ...
            if ([GlobalSettings sharedInstance].searchKey.length > 0) {
                if ([item.position.title rangeOfString:[GlobalSettings sharedInstance].searchKey].location == NSNotFound) {
            ...
            viewToDraw.stationId = item.position.stationId;
            viewToDraw.percorsoId = item.position.percorsoId;
...
- (void)locationManager:(CLLocationManager*)manager didUpdateToLocation:(CLLocation*)newLocation fromLocation:(CLLocation*)oldLocation {
    if (self.locationDelegate && [self.locationDelegate respondsToSelector:@selector(locationManager:didUpdateToLocation:fromLocation:)]) {
        ...
        [GlobalSettings sharedInstance].coordinate = CLLocationCoordinate2DMake(newLocation.coordinate.latitude, newLocation.coordinate.longitude);
    }
}
...
- (void)initInterface {    
    radarView = [[Radar alloc] initWithFrame:CGRectMake(2, 22, 61, 61)];
    radarViewPort = [[RadarViewPortView alloc] initWithFrame:CGRectMake(2, 22, 61, 61)];
    ...
    UILabel *northLabel = [[UILabel alloc] initWithFrame:CGRectMake(28, 12, 10, 10)];
    ...
    valueLabel = [[UILabel alloc] initWithFrame:CGRectMake(8.5, 84, 45, 12)];
    ...
    backToPlugin.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 130, 15, 130, 30);
