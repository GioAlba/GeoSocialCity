...
- (void)openARView {
    ...
    augViewController.locationDelegate = self;
    [self initControls];
    ...
    switch (tabController.selectedIndex) {
        ...
        case 2:
            for (DataSource *data in [_dataSourceManager getActivatedSources]) {
                if ([data.title isEqualToString:@"Facebook"] == NO && [data.title isEqualToString:@"Twitter"] == NO ) {
                    continue;
                }
                checkedSNS = YES;
                break;
...
- (void)createInterface {
    ...
    [cameraButtonDummy setTabBarItem:[[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"Camera", nil) image:[UIImage imageNamed:@"camera.png"] tag:0]];
    
    NSString *sourceTitle = NSLocalizedString(@"Sources", nil);
    ...
    [sourceViewController setTabBarItem:[[UITabBarItem alloc] initWithTitle:sourceTitle image:[UIImage imageNamed:@"icon_datasource.png" ]  tag:1]];
    ...
    //    NSString *listTitle = @"List View";
    NSString *listTitle = NSLocalizedString(@"List View", nil);
    ...
    [listViewController.view setFrame:CGRectMake(0, 50, 320, 480)];
    [listViewController setTabBarItem:[[UITabBarItem alloc] initWithTitle:listTitle image:[UIImage imageNamed:@"list.png"]  tag:2]];
    ...
    NSString *mapTitle = NSLocalizedString(@"Map", nil);
    ...
    [mapViewController setTabBarItem:[[UITabBarItem alloc] initWithTitle:mapTitle image:[UIImage imageNamed:@"map.png"] tag:3]];
...
- (void)showLicense {
    UIAlertView *addAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"License", @"Localizable", [[Resources getInstance] bundle], @"") message:@"Copyright (C) GeoSocial City. Per informazioni visita http://geosocialcity.it/" delegate:self cancelButtonTitle:NSLocalizedStringFromTableInBundle(@"OK", @"Localizable", [[Resources getInstance] bundle], @"") otherButtonTitles:nil, nil];
    [addAlert show];
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    bLat = location.coordinate.latitude;
    bLng = location.coordinate.longitude;
}
...
- (void)locationManager:(CLLocationManager*)manager didUpdateToLocation:(CLLocation*)newLocation fromLocation:(CLLocation*)oldLocation {
    ...
    bLat = newLocation.coordinate.latitude;
    bLng = newLocation.coordinate.longitude;
