...
- (void)convertData:(DataSource*)data currentLocation:(CLLocation*)loc currentRadius:(float)rad {
    ...
    if ([data.title isEqualToString:@"Facebook"] == YES) {
        [data refreshPositionsFB:loc currentRadius:rad];
    }
    else if ([data.title isEqualToString:@"Twitter"] == YES) {
        [data refreshPositionsTW:loc currentRadius:rad];
    }
    else if ([data.title isEqualToString:@"Eventi"] == YES) {
    [data refreshPositionsEventi:[processor convert:[processor createDataString:data.jsonUrl location:loc radius:rad]]];
    }
    else
